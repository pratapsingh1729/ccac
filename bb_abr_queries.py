...
def run_abr_query(cca, abr, utilization_threshold):
    ...
    #         )
    # s.add(Or(*conds))
    s.add(v.S[-1] - v.S[0] < 0.1 * c.C * (c.T - 0))
...
if __name__ == "__main__":
    ...
    #             qres = run_abr_query(cca, abr, utilization_threshold)
    #             if qres is None:
    #                 print("\t\terror\n")
    #             else:
    #                 print(f"\t\t{qres.satisfiable}")
    for utilization_threshold in [0.1]:
        print(f"\n\n\nutilization_threshold = {utilization_threshold}\n")
        for cca in ["aimd"]:
            for abr in ["bulk", "bb_abr"]:
                print(f"cca = {cca}, abr = {abr}")
                result = run_abr_query(cca, abr, utilization_threshold)
                if result is None:
                    print("\t\terror\n")
                else:
